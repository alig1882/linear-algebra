...
\section{Duality}
  \begin{enumerate}[label=\arabic*)]
    ...
    \ii
      ...
      % Let $\Span{\varphi_1, \dots, \varphi_m} = X^{\prime}$. Since $\varphi_1, \dots, \varphi_m$ are linearly independent, they are a
      % basis for $X^{\prime}$. by the next problem, there must exist $v_1, \dots, v_m \in V$ 
      % independent s
      % there must exist $v_1, \dots, v_m$ such that $\varphi_i(v_i) \neq 0$.
      % Now we want to show that $v_1, \dots, v_m$ are linearly independent. Assume this were not the case. Then for some $v_j$, we can re-write it as linear combination of the other elements,
      % say $v_j = \sum_{i \neq j}c_iv_i$. Then 
      % \begin{align*}
      %   \varphi_j(v_j) = \sum_{i \neq j}c_i\varphi_j(v_i) \neq 0.
      % \end{align*}
      % Now $\varphi_j(v_j) \neq 0$ implies that $\varphi_j(\Span{v_j}) = \ran{\varphi_j}$ since certainly $\varphi_j(\Span{v_j}) \subset \ran{\varphi_j}$ and they have the same dimension.
      % Now let $d_i$ be such that $\varphi_j(v_i) = d_i\varphi_i(v_i)$. Then for any $v \in \Span{v_j}$, $v = \lambda v_j$ for some $\lambda \in \F$. Thus 
      % \begin{align*}
      %   \varphi_j(v) & = \varphi_j(\lambda v_j) \\
      %   & = \lambda\varphi_j(v_j) \\
      %   & = \lambda\sum_{i \neq j}c_i\varphi_j(v_i) \\
      %   & = \lambda\sum_{i \neq j}c_id_i\varphi_i(v_i).
      % \end{align*}
      % Furthermore, for all 
      % $\varphi_i(v) \neq 0$, then $\varphi_i(\Span{v}) = \ran{\varphi_i}$. This implies that there exist $v_1, \dots, v_m$ such that $\varphi_i(\Span{v_i}) = \ran{\varphi_i}$
    \ii
